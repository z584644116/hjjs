...
import { calculateFlueGasConversion, roundHalfToEven } from '../fluegas';
...
describe('calculateFlueGasConversion', () => {
  it('should calculate conversion correctly with typical values', () => {
    ...
    const result = calculateFlueGasConversion(150, 6, 13.09);
    
    expect(result).not.toHaveProperty('error');
    if ('convertedConcentration' in result) {
  ...
  it('should return error when measured concentration is missing', () => {
    const result = calculateFlueGasConversion(NaN, 6, 13);
  ...
  it('should return error when reference O2 is missing', () => {
    const result = calculateFlueGasConversion(150, NaN, 13);
    expect(result).toHaveProperty('error');
    if ('error' in result) {
  ...
  it('should return error when measured O2 is missing', () => {
    const result = calculateFlueGasConversion(150, 6, NaN);
    expect(result).toHaveProperty('error');
    if ('error' in result) {
  ...
  it('should return error when reference O2 is out of range (negative)', () => {
    const result = calculateFlueGasConversion(150, -1, 13);
    expect(result).toHaveProperty('error');
    if ('error' in result) {
  ...
  it('should return error when reference O2 is out of range (> 21)', () => {
    const result = calculateFlueGasConversion(150, 22, 13);
    expect(result).toHaveProperty('error');
    if ('error' in result) {
      expect(result.error).toContain('0-21%');
    }
  ...
  it('should return error when measured O2 is >= 21', () => {
    const result = calculateFlueGasConversion(150, 6, 21);
    expect(result).toHaveProperty('error');
    if ('error' in result) {
  ...
  it('should return error when measured concentration is negative', () => {
    const result = calculateFlueGasConversion(-10, 6, 13);
    expect(result).toHaveProperty('error');
    if ('error' in result) {
  ...
  it('should handle zero measured concentration', () => {
    const result = calculateFlueGasConversion(0, 6, 13);
    expect(result).not.toHaveProperty('error');
    if ('convertedConcentration' in result) {
  ...
  it('should handle edge case when reference O2 equals measured O2', () => {
    ...
    const result = calculateFlueGasConversion(100, 10, 10);
    expect(result).not.toHaveProperty('error');
    if ('convertedConcentration' in result) {
  ...
  it('should apply banker\'s rounding correctly', () => {
    ...
    const result = calculateFlueGasConversion(100, 6, 12.5);
    expect(result).not.toHaveProperty('error');
    if ('convertedConcentration' in result) {
